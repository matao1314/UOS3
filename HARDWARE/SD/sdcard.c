...
static u32 StopCondition = 0; 							//是否发送停止传输标志位,DMA多块读写的时候用到
volatile SD_Error TransferError = SD_OK;		//数据传输错误标志,DMA读写时使用
volatile u32 TransferEnd = 0;								//传输结束标志,DMA读写时使用
...
SD_Error SD_PowerON(void)
{
    ...
    else //MMC卡
    {
        ...
        //MMC卡,发送CMD0 SDIO_SEND_OP_COND,参数为:0x80100000
        while((!validvoltage) && (count < SD_MAX_VOLT_TRIAL))
        {
...
SD_Error SD_GetCardInfo(SD_CardInfo *cardinfo)
{
    ...
    if((CardType == SDIO_STD_CAPACITY_SD_CARD_V1_1) || (CardType == SDIO_STD_CAPACITY_SD_CARD_V2_0)) //标准1.1/2.0卡
    {
    ...
    else if(CardType == SDIO_HIGH_CAPACITY_SD_CARD)	//高容量卡
    {
        ...
        tmp = (u8)((CSD_Tab[2] & 0x0000FF00) >> 8); 	//第10个字节
        cardinfo->CardCapacity = (cardinfo->SD_csd.DeviceSize + 1) * 512 * 1024; //计算卡容量
    ...
    else if((SDIO_STD_CAPACITY_SD_CARD_V1_1 == CardType) || (SDIO_STD_CAPACITY_SD_CARD_V2_0 == CardType) || (SDIO_HIGH_CAPACITY_SD_CARD == CardType))
    {
        ...
        else
        {
            ...
            if(SD_OK == errorstatus)
            {
                ...
                SDIO_Clock_Set(SDIO_TRANSFER_CLK_DIV);//设置时钟频率为25Mhz
            }
        }
    }
    return errorstatus;
