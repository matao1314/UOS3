...
void AppTaskStar(void)
{
    OS_ERR err;

    CPU_Init();
    OSInit((OS_ERR	*)&err);/* Init uC/OS-III.*/

    OSTaskCreate(   (OS_TCB     *)&Main_Task_TCB,/* Create the start task*/
                    (CPU_CHAR   *)"Task Start",
                    ...
                    (OS_OPT     )(OS_OPT_TASK_STK_CHK | OS_OPT_TASK_STK_CLR), 
                    (OS_ERR     *)&err);       
    OSStart((OS_ERR	*)&err);/* Start multitasking (i.e. give control to uC/OS-III). */
}
