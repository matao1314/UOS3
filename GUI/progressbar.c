...
_progressbar_obj *progressbar_creat(u16 left, u16 top, u16 width, u16 height, u8 type)
{
    ...
    if(type & PRGB_TYPE_TEXT || (type & PRGB_TYPE_FEN) == 0) {
        progressbar_crt->prgbarlen = 0;    //默认滚条长度为10
    } else {
        progressbar_crt->prgbarlen = 10;    //默认滚条长度为10
    }
...
u8 progressbar_check(_progressbar_obj *prgbx, void *in_key)
{
    ...
    case IN_TYPE_TOUCH:	//触摸屏按下了
        if(prgbx->type & PRGB_TYPE_VER) { //垂直进度条
            ...
            if((temp - key->y) < prgbx->prgbarlen / 2) {
                temp1 = temp;
            } else {
                temp1 = key->y + prgbx->prgbarlen / 2;    //当前x坐标
            }
            prgblen = prgbx->height - prgbx->prgbarlen;
        } else {					   	//水平进度条
            if((key->x - prgbx->left) < prgbx->prgbarlen / 2) {
                temp = 0;
            } else {
                temp = key->x - prgbx->prgbarlen / 2;    //当前x坐标
            }
        ...
        if(prgbx->top <= key->y && key->y <= (prgbx->top + prgbx->height) && prgbx->left <= key->x && key->x <= (prgbx->left + prgbx->width)) {
            if((prgbx->sta & PRGB_BTN_DOWN) == 0) {
                prgbx->sta |= PRGB_BTN_DOWN;    //标记被按下
            }
            if(temp >= temp1) {
                temp -= temp1;
            } else {
...
void progressbar_show_char(u16 x, u16 y, u8 dir, u8 num, u16 color)
{
    ...
    if(dir == 0) {
        ...
        for(t = 0; t < 5; t++) {
            ...
            for(i = 0; i < 8; i++) {
                if(temp & 0x01) {
                    gui_phy.draw_point(x0, y0, color);    //水平
                }
                y0++;
                temp >>= 1;
            }
...
void progressbar_show_percnt(u16 x, u16 y, u8 dir, u8 num, u16 color)
{
    ...
    for(t = 0; t < len; t++) {
        ...
        if(dir == 0) {
            progressbar_show_char(x + 5 * t, y, dir, temp, color);
        } else {
            progressbar_show_char(x, y - 5 * t, dir, temp, color);
        }
    }
    if(dir == 0) {
        progressbar_show_char(x + 5 * t, y, dir, 10, color);    //显示百分号
    } else {
        progressbar_show_char(x, y - 5 * t, dir, 10, color);
    }
}
//画进度浮标
//prgbx:进度条指针
...
void progressbar_setpos(_progressbar_obj *prgbx)
{
    ...
    if((prgbx->type & 0x03) == PRGB_TYPE_TEXT) {
        prgbtlen = 0;    //百分数进度条
    } else if((prgbx->type & 0x03) == PRGB_TYPE_STD) {
        prgbtlen = prgbx->prgbarlen;    //
    }
    ...
    if(prgbx->type & PRGB_TYPE_FEN) {
        progressbar_draw_sign(prgbx, x, y);    //允许画浮标/文字
    }
