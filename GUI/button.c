...
_btn_obj *btn_creat(u16 left, u16 top, u16 width, u16 height, u8 id, u8 type)
{
    ...
    if((type & 0X0f) >= BTN_TYPE_ANG) { //边角按钮/文字按钮
        ...
        btn_crt->bkctbl = NULL;    //对标准按钮和图片按钮这个无效.
    }
    return btn_crt;
}
//删除按钮
...
void btn_delete(_btn_obj *btn_del)
{
    ...
    if((btn_del->type & 0X0f) == BTN_TYPE_TEXTA) {
        btn_recover_backcolor(btn_del);    //恢复背景色
    }
...
void btn_draw_stdbtn(_btn_obj *btnx)
{
    ...
    case BTN_INACTIVE://未激活
        ...
        if(sta == 0) {
            gui_show_strmid(btnx->left, btnx->top, btnx->width, btnx->height, btnx->bcfucolor, btnx->font, btnx->caption);    //显示caption
        } else { //未激活
...
void btn_draw_arcbtn(_btn_obj *btnx)
{
    ...
    u8 sta;
    if(btnx->arcbtnr > btnx->width / 2 || btnx->arcbtnr > btnx->height / 2) {
        return;    //超过范围了
    }
    sta = btnx->sta & 0x03;
    switch(sta) {
...
void btn_draw(_btn_obj *btnx)
{
    ...
    if(btnx->sta & (1 << 6)) {
        if((btnx->sta & 0x03) == BTN_PRESS) {
            btn_press_hook(btnx);    //按键按下的钩子函数
        } else if((btnx->sta & 0x03) == BTN_RELEASE) {
            btn_release_hook(btnx);    //按键松开时的钩子函数
        }
    }
}
