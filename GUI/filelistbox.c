...
_filelistbox_obj *filelistbox_creat(u16 left, u16 top, u16 width, u16 height, u8 type, u8 font)
{
    ...
    if(height <= FLBOX_PATH_HEIGHT + FLBOX_ITEM_HEIGHT + FLBOX_FINFO_HEIGHT) {
        return NULL;    //高度小了
    }
    if((height - FLBOX_PATH_HEIGHT - FLBOX_FINFO_HEIGHT) % FLBOX_ITEM_HEIGHT) {
        return NULL;    //尺寸不合格
    }
    if(width < FLBOX_ITEM_HEIGHT) {
        return NULL;    //尺寸不合格
    }
    filelistbox_crt = (_filelistbox_obj *)gui_memin_malloc(sizeof(_filelistbox_obj));	//分配内存
    if(filelistbox_crt == NULL) {
        return NULL;    //内存分配不够.
    }
...
_filelistbox_list *filelist_search(_filelistbox_list *filelistx, u16 index)
{
    ...
    while(filelisttemp->prevlist != NULL) {
        filelisttemp = filelisttemp->prevlist;    //追溯到最开始的list
    }
...
u8 filelistbox_rebuild_filelist(_filelistbox_obj *filelistbox)
{
    ...
    if(filelistbox != NULL) {
        ...
        if(filelistbox->scbv->totalitems > filelistbox->scbv->itemsperpage) {
            filelistbox->scbv->topitem = filelistbox->selindex;
        }
        ...
        if(filelistbox != NULL) { //listbox非空
            ...
            if((filelistbox->top + FLBOX_PATH_HEIGHT) <= key->y && key->y <= (filelistbox->top + filelistbox->height - FLBOX_FINFO_HEIGHT) && filelistbox->left <= key->x && key->x < (endx)) { //在items区域内
                ...
                for(i = 0; i < filelistbox->scbv->itemsperpage; i++) { //找到当前触屏按下的坐标在listbox种的index
                    if(key->y <= (filelistbox->top + FLBOX_PATH_HEIGHT + (i + 1)*FLBOX_ITEM_HEIGHT) && key->y >= (filelistbox->top + FLBOX_PATH_HEIGHT + i * FLBOX_ITEM_HEIGHT)) {
                        break;
                    }
                }
                if((filelistbox->sta & (1 << 6)) == 0) { //编号还无效
                    ...
                    if(gui_disabs(i, (filelistbox->sta & 0x3f)) > 1)	{
                        filelistbox->sta |= 1 << 7;    //滑动距离大于1个条目的间隙 标记滑动
                    }
                }
                if((filelistbox->sta & 0xc0) == 0xc0) { //是滑动
                    ...
                    if(tempindex > i) { //减少了
                        ...
                        if(filelistbox->scbv->topitem >= (filelistbox->scbv->totalitems - filelistbox->scbv->itemsperpage)) {
                            filelistbox->scbv->topitem = filelistbox->scbv->totalitems - filelistbox->scbv->itemsperpage;
                        }
                    } else if(i > tempindex) { //增加了
                        filelistbox->sta &= 0xc0; //清空上次的
                        filelistbox->sta |= i; //记录最近的index号
                        i -= tempindex;
                        if(filelistbox->scbv->topitem >= i) {
                            filelistbox->scbv->topitem -= i;
                        } else {
                            filelistbox->scbv->topitem = 0;
                        }
                    } else {
                        break;
                    }
                    if(filelistbox->scbv->topitem != lastindex) {
                        filelistbox_draw_listbox(filelistbox);    //重画listbox
                    }
                }
                ...
                if(filelistbox->sta & (1 << 7)) { //滑动
                    ...
                    //双击目标文件了.
                    if(filelistbox->dbclick == 0X81 && filelistbox->list != NULL) {
                        filelist_delete(filelistbox);    //删除之前的条目,释放内存
                    }
                    filelistbox->oldtime = GUI_TIMER_10MS; //记录当前时间
                    ...
                    if(gui_disabs(filelistbox->oldtime, GUI_TIMER_10MS) >= 10) { //超过100ms了.
                        if(filelistbox->selindex >= filelistbox->scbv->topitem && filelistbox->selindex < (filelistbox->scbv->topitem + filelistbox->scbv->itemsperpage)) { //选中条目在当前的显示范围内
                            ...
                            if(i < filelistbox->namelen) { //小于文字长度,就是不能够一次显示的
                                ...
                                if(filelistbox->curnamepos + i > filelistbox->namelen + i / 4) {
                                    filelistbox->curnamepos = 0;    //循环显示
                                }
                            }
                        }
                        filelistbox->oldtime = GUI_TIMER_10MS; //记录当前时间
...
u8 filelistbox_addlist(_filelistbox_obj *filelistbox, u8 *name, u8 type)
{
    ...
    if(filelistbox->list == NULL) { //还未创建链表
        ...
        while(filelisttemp->nextlist != NULL) {
            filelisttemp = filelisttemp->nextlist;    //偏移到最后一个list
        }
    ...
    if(filelistbox->scbv->totalitems > filelistbox->scbv->itemsperpage) {
        filelistbox->type |= 0x80;    //需要显示滚条
    } else {
        filelistbox->type &= ~0x80;    //不需要显示滚条
    }
    return 0;
}
//画filelist
...
void filelistbox_show_path(_filelistbox_obj *filelistbox)
{
    ...
    if(strlen((const char *)filelistbox->path) > 4) {
        offset = 1;    //文件夹
    } else {
        offset = 0;    //借用一下offset
    }
...
void filelistbox_show_info(_filelistbox_obj *filelistbox)
{
    ...
    if(filelistx->type == FICO_DISK) { //当前选中的是磁盘
        ...
        if(filelistbox->selindex < filelistbox->foldercnt) {
            fresec = filelistbox->selindex + 1;    //在文件夹总数目以内.
        } else {
            freclust = filelistbox->selindex - filelistbox->foldercnt + 1;    //在文件总数目以内
        }
...
void filelistbox_draw_listbox(_filelistbox_obj *filelistbox)
{
    ...
    if(filelistbox->scbv->totalitems > filelistbox->scbv->itemsperpage) {
        scrollbar_draw_scrollbar(filelistbox->scbv);    //必要时,画滚动条
    }
...
u8 filelistbox_scan_filelist(_filelistbox_obj *filelistbox)
{
    ...
    if(filelistbox->path) {	//路径有效
        ...
        while(res == FR_OK) {
            ...
            if (flboxfinfo.fname[0] == '.' && flboxfinfo.fname[1] == 0) {
                continue;    //忽略本级目录
            }
            ...
            if(flboxfinfo.fattrib & (1 << 4) && mark == 0) { //是一个文件夹且mark=0
                ...
                if(((1 << type)&filelistbox->fliter) && ((1 << typeex)&filelistbox->fliterex)) { //大小类型都正确
                    ...
                    if(filelistbox->filecnt < FLBOX_MAX_DFFILE) {
                        filelistbox->findextbl[filelistbox->filecnt] = findex;    //记录下文件索引
                    }
            ...
            if(filelistbox_addlist(filelistbox, fn, type)) {
                break;    //添加失败了
            }
        }
        if(mark == 1 && filelistbox->filecnt) {
            ...
            if(fn != NULL) {
                filelistbox->findextbl = (u16 *)fn;    //如果申请成功了,则使用新的地址,否则源地址不变
            }
        }
    ...
    if(filelistx->type == 0 || filelistx->type == 1) { //属于磁盘/文件夹,则打开
        ...
        if(filelistx->type == 0) { //是磁盘
            ...
            if(strcmp((const char *)filelistx->name, "..")) { //不是..文件名
                if(temp < FLBOX_PATH_DEPTH) {
                    filelistbox->seltbl[temp] = filelistbox->selindex;    //记录下当前目录下选中的
                }
